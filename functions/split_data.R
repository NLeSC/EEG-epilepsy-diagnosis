  ...
  split_data = function(LAB,DAT,proto_i,split=c(20,20)) {
    ...
    for (set in 1:2) { # first select test and validation set based on split as provided in the arguments
      for (diag_i in c(1,2)) { #c("Control","Epilepsy")) {
        x = LAB[which(LAB$protocol == proto_i & LAB$diagnosis == diag_i & (LAB$id %in% ids) == FALSE),]$id
        if (length(x) == 0) {
...
#   turn2charfactor = function(x) {
#     x = as.character(x)
#     x[which(x == "1")] = "Control"
#     x[which(x == "2")] = "Epilepsy"
#     x = as.factor(x)
#   }
#   DATtrain$diagn = DATtest$diagn = DATval$diagn = as.factor("NA")
#   DATtest$diagn = turn2charfactor(LABtest$diagnosis)
#   
#   DATtrain$diagn = turn2charfactor(LABtrain$diagnosis)
#   DATval$diagn = turn2charfactor(LABval$diagnosis)
#   print(DATtest$diang)
#   lll
  DATtest$diagn = LABtest$diagnosis
  DATtrain$diagn = LABtrain$diagnosis
  DATval$diagn = LABval$diagnosis
  invisible(list(LABval=LABval,LABtest=LABtest,LABtrain=LABtrain,DATval=DATval,DATtest=DATtest,DATtrain=DATtrain)) 
}